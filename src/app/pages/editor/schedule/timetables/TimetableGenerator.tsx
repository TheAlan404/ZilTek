...
const generateTimetable = (segments: TimetableGenSegment[]) => {
    ...
    segments.forEach(segment => {
        if (segment.type == "startTime")
            ...
            currentTime = AddTime(currentTime, segment.offset || 0);

        for (let i = 0; i < segment.classCount; i++) {
            let tuple: Tuple = [];
            if (segment.studentBellOffset) {
                tuple.push({
                    value: SubtractTime(currentTime, segment.studentBellOffset || 0),
                    variant: "idle",
                });
            } else {
                tuple.push({
            ...
            currentTime = AddTime(currentTime, segment.classDuration || 0);
            tuple.push({
                value: currentTime,
                variant: "idle",
            });
            if (i < segment.classCount - 1) {
                currentTime = AddTime(currentTime, segment.breakDuration || 0);
            };
